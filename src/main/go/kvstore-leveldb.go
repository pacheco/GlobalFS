...
// synchronization.
var storeDB *levigo.DB
var readOpt = levigo.NewReadOptions()
var writeOpt = levigo.NewWriteOptions() // writeOpt.SetSync(bool) defaults to false
...
func storageGet(key []byte) ([]byte, error) {
	data, err := storeDB.Get(readOpt, key)
	if err != nil {
		return nil, err
	} else {
		return data, nil
